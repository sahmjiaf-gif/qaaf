...
class AppErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean, error: any }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-stone-100 p-6 text-center" dir="rtl">
          <div className="max-w-md w-full bg-white rounded-[2rem] shadow-2xl border border-slate-200 p-8">
            <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-red-100 text-2xl">⚠️</div>
            <h1 className="text-2xl font-black text-slate-900">حدث خطأ أثناء تحميل الموقع</h1>
            <p className="mt-3 text-sm text-slate-600">تمت حماية التطبيق من التعطل الكامل، ويمكنك إعادة المحاولة الآن.</p>
            <button
              onClick={() => window.location.reload()}
              className="mt-6 inline-flex items-center justify-center rounded-xl bg-slate-900 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-slate-200 transition hover:bg-slate-700"
            >
              تحديث الصفحة
            </button>
            <pre className="mt-5 hidden max-h-40 overflow-auto rounded-xl bg-slate-100 p-3 text-left text-[10px] text-red-700">
